...
Texture2D g_txDepth;
Texture2D g_txScreen;
Texture2D g_txMask;
Texture2D g_txGodRay;
...
PSSceneIn VSLightmain(VSSceneIn input)
{
    PSSceneIn output;
    
    output.vPos = mul( float4(input.Pos*5+g_vLightPos,1), g_mWorld );
    output.Pos = mul( float4(input.Pos*5+g_vLightPos,1), g_mWorldViewProj );
    output.Norm = normalize( mul( input.Norm, (float3x3)g_mWorld ) );
    output.Tan = normalize( mul( input.Tan, (float3x3)g_mWorld ) );
    output.Tex = input.Tex;
...
float4 PSLightmain(PSSceneIn input) : SV_Target
{   
    return float4(1,1,0.7,1);
}
...
float4 PSMaskmain(PSSceneIn input) : SV_Target
{   
return 0;
}
//
...
{   
    ...
    // diffuse lighting
    float4 lighting = saturate( dot( norm,g_vLightPos /*g_vWorldLightDir1.xyz*/ ) )*g_directional1;
    lighting += saturate( dot( norm, g_vWorldLightDir2.xyz ) )*g_directional2;
    lighting += g_ambient;
    
    // Calculate specular power
    float3 viewDir = normalize( g_vEyePt - input.vPos );
    float3 halfAngle = normalize( viewDir + g_vLightPos.xyz );
    ...
    return 0.2*(lighting*diffuse + (specPower1+specPower2)*specMask);
}
...
{   
    return 0.2*g_txDiffuse.Sample( g_samLinearWrap, input.Tex );
}
...
{     
    ...
    particleSample.rgb *= Light.xyz*input.particleColor*0.5;
    particleSample.a *= depthFade;
    
    return particleSample;
}
float4 PSBillboardParticleMaskmain(PSParticleIn input, uniform bool bSoftParticles, uniform bool bUseMSAA ) : SV_TARGET
{     
    float2 screenTex = 0.5*( (input.ScreenTex) + float2(1,1));
    screenTex.y = 1 - screenTex.y;
    ...
    particleSample.a *= depthFade;
    //particleSample.a-=0.3;
    particleSample.a*=2;
    return float4(0,0,0,particleSample.a);
}
    ...
    VSSceneIn input )
{
    ...
    output.Pos = float4(input.Pos,1.0);
    output.Tex = input.Tex;
	float4 mat=mul(float4(g_vLightPos,1),g_mWorldViewProj);
	output.Tan.xy=mat.xy/mat.w*float2(0.5,-0.5)+0.5;
    //
    return output;
}
...
{    
    return g_txScreen.Sample( g_samLinear, input.Tex );
...
{    
    return g_txScreen.Sample( g_samLinear, input.Tex )+g_txGodRay.Sample( g_samLinear, input.Tex );
...
{    
   float2 LightPos=input.Tan.xy;
   float2 deltaCoord=LightPos-input.Tex;
 ...
 float decay=0.99;
   float exposure=0.5;
  deltaCoord*=1.0f/NumOfSample*density;
   
   float brightness=1.0f;
  // tex=tex*0.8+0.1;
   
   float4 c=g_txMask.Sample(g_samLinearWrap,input.Tex); 
 
   for(int i=0;i<NumOfSample;i++)
   {
      tex+=deltaCoord;
      float4 samples=g_txMask.Sample(g_samLinearWrap,tex); 
      samples*=brightness*weight;
      c+=samples;
      brightness*=decay;
   }
...
technique10 RenderLight
{
    pass p0
    {
        SetVertexShader( CompileShader( vs_4_0, VSLightmain() ) );
        SetGeometryShader( NULL );
        SetPixelShader( CompileShader( ps_4_0, PSLightmain() ) );
...
technique10 RenderSceneMask
{
    pass p0
    {
        SetVertexShader( CompileShader( vs_4_0, VSScenemain() ) );
        SetGeometryShader( NULL );
        SetPixelShader( CompileShader( ps_4_0, PSMaskmain() ) );
...
technique10 RenderGodRay
{
    pass p0
    {
        SetVertexShader( CompileShader( vs_4_0, VSGRQuad() ) );
        SetGeometryShader( NULL );
        SetPixelShader( CompileShader( ps_4_0, PSGRQuad() ) );
...
technique10 RenderBillboardParticlesToMask_Soft
{
    pass p0
    {
        SetVertexShader( CompileShader( vs_4_0, VSParticlemain() ) );
        SetGeometryShader( CompileShader( gs_4_0, GSParticlemain() ) );
        SetPixelShader( CompileShader( ps_4_0, PSBillboardParticleMaskmain(true,false) ) );
...
technique10 RenderComb
{
    pass p0
    {
        SetVertexShader( CompileShader( vs_4_0, VSQuad() ) );
        SetGeometryShader( NULL );
        SetPixelShader( CompileShader( ps_4_0, PSCQuad( ) ) );
