...
ID3D10InputLayout*      g_pScreenVertexLayout = NULL;
...
ID3D10EffectTechnique*  g_pRenderLight = NULL;
...
ID3D10EffectShaderResourceVariable* g_pDScreenTex = NULL;
...
PARTICLE_TECHNIQUE g_ParticleTechnique = PT_BILLBOARD_SOFT;
...
// PL Defined
//--------------------------------------------------------------------------------------
ID3D10Texture2D*                    g_pColorTexture[3]             = {NULL};
ID3D10RenderTargetView*             g_pColorRTView[3]              = {NULL};
ID3D10ShaderResourceView*           g_pColorSRView[3]              = {NULL};
ID3D10EffectTechnique*              g_pRenderVerticiesQuad      = NULL;
ID3D10EffectTechnique*              g_pRenderSceneToMask      = NULL;
ID3D10EffectTechnique*              g_pRenderParticleToMask      = NULL;
ID3D10EffectTechnique*              g_pRenderGodRay      = NULL;
ID3D10EffectTechnique*              g_pRenderComb      = NULL;
ID3D10Buffer*                       g_pFullScreenVertexBuffer   = NULL;
ID3D10EffectShaderResourceVariable* g_pTex[3]               = {NULL};
D3DXVECTOR3 vecLight3=D3DXVECTOR3(6,8,20);
bool animated=false;
bool oldVersion=true;
float time=3.1415926*3/2;
ID3D10EffectVectorVariable* g_pvLightPt = NULL;
...
#define IDC_ANIMATION          7
#define IDC_VERSION          8
...
void InitApp()
{
    ...
    g_SampleUI.AddCheckBox(IDC_ANIMATION,L"Animation",35,iY,125,22,false,'a');
    g_SampleUI.AddCheckBox(IDC_VERSION,L"New Version",35,iY+=25,125,22,false,'v');
...
HRESULT CALLBACK OnD3D10CreateDevice( ID3D10Device* pd3dDevice, const DXGI_SURFACE_DESC *pBackBufferSurfaceDesc, void* pUserContext )
{
    ...
    g_pRenderLight = g_pEffect10->GetTechniqueByName( "RenderLight" );
    g_pRenderGodRay = g_pEffect10->GetTechniqueByName( "RenderGodRay" );
    g_pRenderComb = g_pEffect10->GetTechniqueByName( "RenderComb" );
	//------------------------------------------------------------
    ...
    g_pRenderVerticiesQuad->GetPassByIndex( 0 )->GetDesc( &PassDesc );
    ...
    hr = pd3dDevice->CreateInputLayout( 
        ...
        &g_pScreenVertexLayout );

    V_RETURN( hr );
	//------------------------------------------------------------
    g_pRenderSceneToMask = g_pEffect10->GetTechniqueByName( "RenderSceneMask" );
    g_pRenderParticleToMask = g_pEffect10->GetTechniqueByName( "RenderBillboardParticlesToMask_Soft" );
    ...
    g_pvLightPt = g_pEffect10->GetVariableByName( "g_vLightPos" )->AsVector();
    ...
    g_pDepthMSAATex = g_pEffect10->GetVariableByName( "g_txDepthMSAA" )->AsShaderResource();
	 ...
	 g_pTex[1]                 = g_pEffect10->GetVariableByName( "g_txScreen" )->AsShaderResource();
	 g_pTex[0]                 = g_pEffect10->GetVariableByName( "g_txMask" )->AsShaderResource();
	 g_pTex[2]                 = g_pEffect10->GetVariableByName( "g_txGodRay" )->AsShaderResource();
    ...
    V_RETURN( g_BallMesh.Create( pd3dDevice, L"SoftParticles\\ball.sdkmesh", false ) );
...
void CALLBACK OnD3D10FrameRender( ID3D10Device* pd3dDevice, double fTime, float fElapsedTime, void* pUserContext )
{
    float ClearColor[4] = { 0.3, 0.3, 0.18, 1.0 };
	if(oldVersion)
	{
		ClearColor[0]=0;
		ClearColor[1]=0;
    ...
    pd3dDevice->ClearRenderTargetView( g_pColorRTView[0], ClearColor);
	...
	pd3dDevice->OMSetRenderTargets( 1, &g_pColorRTView[1], g_pDepthStencilDSV );
    ...
    g_pvLightPt->SetFloatVector( (float*)&vecLight3 );
    ...
    g_BallMesh.Render( pd3dDevice, g_pRenderLight);
        ...
        PT_BILLBOARD_ODEPTH != g_ParticleTechnique )
    {
        ...
        pd3dDevice->OMSetRenderTargets( 1, &g_pColorRTView[1], NULL );
	...
	pd3dDevice->OMSetRenderTargets( 1, &g_pColorRTView[0], NULL);
	    g_BallMesh.Render( pd3dDevice, g_pRenderLight);
	    g_ObjectMesh.Render( pd3dDevice, g_pRenderSceneToMask);




		 pParticleTech = g_pRenderParticleToMask;
		    if( PT_BILLBOARD_HARD != g_ParticleTechnique &&
        PT_BILLBOARD_ODEPTH != g_ParticleTechnique )
    {
        // Unbind the depth stencil texture from the device
	...
	pd3dDevice->OMSetRenderTargets(1,&g_pColorRTView[2],NULL);
	pd3dDevice->IASetInputLayout( g_pScreenVertexLayout );
	g_pTex[0]->SetResource(g_pColorSRView[0]);
pd3dDevice->IASetVertexBuffers(
        ...
        &uStride,
        &uOffset );
	g_pRenderGodRay->GetPassByIndex( 0 )->Apply( 0 );
	...
	g_pTex[1]->SetResource(g_pColorSRView[1]);
	g_pTex[2]->SetResource(g_pColorSRView[2]);
	g_pRenderComb->GetPassByIndex( 0 )->Apply( 0 );
...
void CALLBACK OnD3D10SwapChainReleasing( void* pUserContext )
{
	...
	for(int i=0;i<3;i++)
	{
    SAFE_RELEASE( g_pColorTexture[i] );
    SAFE_RELEASE( g_pColorRTView[i] );
    SAFE_RELEASE( g_pColorSRView[i] );
	}
}
...
void CALLBACK OnD3D10DestroyDevice( void* pUserContext )
{
    ...
    SAFE_RELEASE( g_pScreenVertexLayout );
